	...
	void MDX::write_MTLS_chunk(BinaryWriter& writer) const {
		...
		for (const auto& material : materials) {
			...
			for (const auto& layer : material.layers) {
				...
				writer.write<uint32_t>(layer.layerTextures.size());


				for (size_t i = 0; i < layer.layerTextures.size(); i++) {
					writer.write<uint32_t>(layer.layerTextures[i].id);
					writer.write<uint32_t>(i);
					layer.layerTextures[i].KMTF.save(TrackTag::KMTF, writer);
				}
